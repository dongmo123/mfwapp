        ...
        var P = new h({
            ...
            handleSuggest: function(aB) {
                ...
                if (!!ab) {
                    ...
                    aj.append('<div class="mss-title">鎼溾€�<span class="mss-key">' + aw + '</span>鈥濈浉鍏虫父璁�<span class="mss-num">' + ab + "绡�</span></div>")
                }
                var am = aB.user_num;
                if (!!ab) {
                    var aj = $('<li class="mss-item _j_listitem" data-type="user">').appendTo(aq);
                    aj.append('<div class="mss-title">鎼溾€�<span class="mss-key">' + aw + "</span>鈥濈浉鍏崇敤鎴�</div>")
                }
                if (s > 0) {
...
M.define("jqui-datepicker", function(d) {
    ...
    $.extend(c.prototype, {
        ...
        _generateHTML: function(Z) {
            var B, A, U, L, m, ae, X, Q, ah, J, al, t, v, u, j, ad, ac, r, E, ag, S, am, D, I, s, n, V, O, R, P, q, G, w, Y, ab, l, af, aj, N, x, aa = new Date(),
    ...
    var c = function(g) {
        ...
        this.suggestion = new f({
            ...
            handleSuggest: M.bind(function(j) {
                ...
                if (k) {
                    k += '<a class="ssp-more _j_listitem _j_listitem_default" href="javascript:;">鏄剧ず鍏ㄩ儴鎼滅储缁撴灉</a></div>'
                }
                return k
            }, this),
...
M.closure(function(h) {
    ...
    var aa = h("Slider"),
        ...
        e = h("MesSearchEvent"),
        ac = h("xdate"),
        g = h("/js/hotel/module/DateRangePicker"),
        k = h("/js/hotel/module/BookingDate"),
        m = k.getDate(true),
        S = m.checkIn,
        K = m.checkOut;
    var n = 600,
        X = 5000,
        aj = false,
        V = $("#_j_top_pic_container"),
        ak = V.find(".show-image"),
        i = V.find(".show-nav"),
        ap = ak.children("li"),
        z = ap.length;
    var J = new aa({
    ...
    ak.add(i).bind("mouseenter", function() {
        aj = true
    ...
    ak.delegate("li", "click", function() {
        var az = $(this);
        try {
            var av = az.find("a").attr("href");
            var ay = av.split("/");
            var ax = parseInt(ay[ay.length - 1]);
            mfwPageEvent("ginfo", "index:focus:image:click", ({
                iid: ax
            }))
        } catch (aw) {}
    });
    var b = $("#_j_index_search"),
        Y = $("#_j_index_search_tab"),
        I = $("#_j_index_search_bar_all"),
        q = $("#_j_index_search_bar_hotel"),
        ae = $("#_j_index_search_bar_mdd"),
        v = $("#_j_index_search_bar_sales"),
        j = function(av) {
            switch (av) {
                case 0:
                    return I;
    ...
    Y.on("click", "li", function(ay) {
        var aA = $(ay.currentTarget),
            av = Number(aA.data("index")),
            aw = j(av),
            aC = Y.find(".tab-selected"),
            aB = Number(aC.data("index")),
            az = j(aB),
            ax = $.trim(az.find(".search-input input").val());
        Y.find("li").removeClass("tab-selected");
        aA.addClass("tab-selected");
        b.find(".searchbar").hide();
        aw.find(".search-input input").val(ax).end().show()
    });
    var w = true;
    var U = function(aw, aB, az, ay, ax) {
        var av = {};
        if (aB == "hotel") {
            av.url = "/hotel/s.php?q=" + encodeURIComponent(aw)
        } else {
            if (aB == "sales") {
                av.url = "/sales/0-0-K" + encodeURIComponent(aw) + "-0.html"
            } else {
                av.url = "/search/s.php?q=" + encodeURIComponent(aw)
            }
        }
        av.search_type = "all";
        av.search_category = "suggest";
        av.trigger = ay;
        av.search_from = "index_" + aB;
        av.keyword = aw;
        if (!!az) {
            var aA = az.closest("li");
            av.local = aA.index()
        }
        if (az.length) {
            av.url = az.data("url");
            av.search_type = az.data("type")
        }
        av.trigger = av.search_type == "more" ? "more" : av.trigger;
        if (av.trigger != "enter") {
            w = false
        }!!mfwPageEvent && mfwPageEvent("se", "result_click", av)
    };
    new af({
    ...
    E.on("click", function(av) {
        t.datepicker("show")
    });
    ah.on("click", function(av) {
        f.datepicker("show")
    });
    M.Event(H).on("date range picker doublepicked", function(av) {
        if ("" === t.val()) {
            t.val(av.startDate)
        }
        if ("" === f.val()) {
        ...
        k.setDate(av.startDate, av.endDate, "homepage")
    });
    var L = h("/js/hotel/module/Search_v2"),
        D = $("#_j_index_search_input_hotel"),
        p = $("#_j_index_search_btn_hotel"),
        an = $("#_j_index_suggest_list_hotel"),
        R = $("#_j_index_search_bar_hotel form"),
        Q = new L({
            input: "_j_index_search_input_hotel",
            listContainer: "_j_index_suggest_list_hotel",
            defaultSearch: function(aw, av) {
                var ax = "/hotel/s.php?keyword=" + aw;
                if (!!av) {
                    ax += "&disautomatch=1"
                }
    ...
    M.Event(Q).on("itemselected", function(av) {
        if (!av.item.hasClass("_j_listitem_default")) {
            U($.trim(D.val()), "hotel", av.item, "click")
        }
    });
    p.click(function(aw) {
        var av = $.trim(D.val());
        if (!!av) {
            U(av, "hotel", "", "search_btn");
            location.href = "/hotel/s.php?keyword=" + encodeURIComponent(av)
        }
    });
    R.on("submit", function() {
        ...
        U($.trim(D.val()), "hotel", "", "enter")
    });
    var G = $("#_j_index_search_input_mdd"),
        al = $("#_j_index_search_btn_mdd"),
        T = $("#_j_index_suggest_list_mdd"),
        x = $("#_j_index_search_bar_mdd form");
    var ad = new l({
        ...
        listContainer: T,
        handleSuggest: function(aB) {
            G.data("droplist")["firstItemHover"] = aB.data.exact;
            var ax = $.trim(G.val());
            if (!aB.data.mdd.length) {
                G.data("droplist").hide()
            } else {
                var aw = aB.data.mdd,
                    az = [];
                for (var aA = 0, av = aw.length, aC = ""; aA < av; aA++) {
                    var ay = "/travel-scenic-spot/mafengwo/" + aw[aA].mddid + ".html";
                    az.push('<li class="_j_listitem" data-type="mdd" data-url="' + ay + '"><i class="sicon-place"></i><span class="skey">' + aw[aA].name + "</span>" + (aw[aA].parent ? ("<span>" + aw[aA].parent + "</span>") : "") + "</li>")
                }
                return az.join("")
            }
        },
        updateList: function(av) {
            this.listContainer.find(".suggest-list").html(av)
        }
    });
    M.Event(ad).on("itemselected", function(av) {
        var aw = av.item;
        if (aw.length) {
            U($.trim(G.val()), "mdd", aw, "click");
            location.href = aw.data("url")
        } else {
            if (G.parents("form").length) {
                G.parents("form").last().submit()
            } else {
                return
            }
        }
    });
    al.click(function(aw) {
        var av = $.trim(G.val());
        if (!!av) {
            U(av, "mdd", "", "search_btn");
            location.href = "/search/s.php?q=" + encodeURIComponent(av)
        }
    });
    x.on("submit", function() {
        ...
        U($.trim(G.val()), "mdd", "", "enter")
    });
    var P = $("#_j_index_search_input_sales"),
        u = $("#_j_index_search_btn_sales"),
        ai = function() {
            var av = $.trim(P.val());
            if (!!av) {
                var aw = {
                ...
                var ax = e.search(aw);
                U(av, "sales", "", "search_btn");
                location.href = "/search/s.php?t=sales&q=" + encodeURIComponent(av) + "&seid=" + ax
            }
        };
    u.click(function(av) {
        ai()
    });
    P.on("keyup", function(av) {
        if (av.keyCode == 13) {
            ai()
        }
    });
    var B = $("#_j_traveller_slide"),
        s = B.find(".slide-ul"),
        ag = s.children("li"),
        N = ag.length,
        c = B.find(".slide-ol li"),
        C = 300,
        a = 10000;
    var d = new aa({
        slideCnt: s,
    ...
    c.bind("click", function(av) {
        clearInterval(F)
    });
    var o = true,
        au = 1,
        Z, ao = null,
        ab = $("._j_activitynav").html();
    var W = new aq({
    ...
    $("._j_activity").click(function(av) {
        av.preventDefault();
        var aw = $(av.currentTarget),
            ax = aw.data("type");
        if (ax == au) {
            return
        }
        ...
        if (au == 1) {
            ...
            aw.addClass("active");
            M.Event(W).fire("contentchange");
            W.scroll(0)
        } else {
            if (o && !Z) {
                o = false;
                ao = $.get("/indexactivity/ajax.php", {
                    act: "mfwIndexActivity",
                    type: au
                }, function(ay) {
                    if (ay.data) {
                        Z = ay.data.html;
    ...
    var A = h("dialog/Dialog"),
        at;
    $("._j_activitynav").delegate("._j_wx_open", "click", function(aw) {
        if (!at) {
            at = new A({
                content: '<div class="wx-mfw-pop"><p>浜茬埍鐨勮渹铚傦紝璇ユ椿鍔ㄤ负鎵嬫満娲诲姩</p>蹇壂鎻忎笅闈㈢殑浜岀淮鐮佸弬涓庡惂<p></p><p><img class="_j_new_url" width="150" height="150" src=""></p></div>'
            });
            at.show()
        }
        var av = $(aw.currentTarget).data("url");
        $("._j_new_url").attr("src", av);
        at.show()
    });
    var r = $("#_j_sales");
    r.on("mouseenter", ".item", function(ax) {
        var aw = $(ax.currentTarget),
            av = aw.find("h3");
        aw.addClass("hover");
        av.stop().animate({
            height: 80,
    ...
    }).on("mouseleave", ".item", function(ax) {
        var aw = $(ax.currentTarget),
            av = aw.find("h3");
        aw.removeClass("hover");
        av.stop().animate({
            height: 40,
            marginTop: 5
        }, {
    ...
    var ar = $("._j_tn_filter"),
        y = $("#_j_tn_content");
    ar.each(function(av, aw) {
        var ax = $(aw);
        new O({
            nav: ax,
            panel: ax.find(".dropdown-menu ul"),
            showCallback: function(ay, az) {
                az.closest(".tn-dropdown").addClass("dropdown-open")
            },
            hideCallback: function(ay, az) {
                az.closest(".tn-dropdown").removeClass("dropdown-open")
            }
        })
    });
    (function() {
        ...
        $("#_js_fsStep2").delegate(".i_know", "click", function(av) {
    ...
    (function() {
        var av = $("._js_header_lantern");
        av.each(function(ax, ay) {
            var aw = $(ay);

            function az() {
                aw.toggleClass("hover")
            }
            aw.data("timer", setInterval(az, 1000));
            aw.hover(function() {
                clearInterval(aw.data("timer"));
                aw.data("timer", null)
            }, function() {
                aw.data("timer", setInterval(az, 1000))
            })
            ...
            var l = new c({
                ...
                handleSuggest: function(p) {
                    ...
                    if (p && p.payload && p.payload.data && p.payload.data.length > 0) {
                        ...
                        var m = $.trim(e.val());
                        o.push('<li class="not_found" data-name="ssss"><div class="nf_title">娌℃湁鎵惧埌<span>鈥�' + m + "鈥�</span>!</div></li>")
                    }
                    return o.join("")
                },
            });
    ...
    d.prototype = {
        ...
        polling: function() {
            ...
            var h = $.param({
                filter: new e("req.polling", {
                    user: {
                        c_uid: this.pollingParams.c_uid ? this.pollingParams.c_uid : window.IM_CONFIG.user.uid
                    },
                    message: {
                        max_id: this.pollingParams.max_id,
                        read_id: this.pollingParams.read_id
                    },
                    conn: {
                    ...
                    thread: {
                        type: this.pollingParams.thread_type,
                        info: this.pollingParams.thread_info
                    },
                    session: {
                        conn_id: this.pollingParams.conn_id
                    }
                })
            });
        ...
        getMulti: function(f, n, j) {
            ...
            var m = $.getJSON(i.url, g, function(o) {
                if (j) {
                    if (l._ticket[f.e] != k) {
                        i.log("蹇界暐璇锋眰:" + f.e + "|" + f.e, f);
                        return
                    }
                }
                if (!o || !("errno" in o)) {
                    i.alert(f.e + "锛氱綉缁滈敊璇�1", "danger")
                } else {
                    if (o.errno != 0) {
        ...
        postWxQrCode: function(f) {
            var g = {
                qrcode: {
                    c_uid: f
                }
            };
            return this.getMulti(new e("req.wx.request.qrcode", g), "", true)
        },
        ...
        groupInfo: function(f) {
            return this.getMulti(new e("req.group.info", {
                id: f
            }))
        },
        ...
        groupUserList: function(f) {
            return this.getMulti(new e("req.group.user.list", {
                group: {
                    mid: f
                }
            }))
        ...
        groupApply: function(h, g, f) {
            return this.post(new e("req.group.apply", {
                mid: h,
                mdd: g,
                date: f
            }))
        },
        evaluateBusiness: function(g, f) {
            return this.post(new e("req.evaluate.business", {
                evaluate: {
                    evaluate_score: g,
                    evaluate_note: f
                },
        ...
        postGroupMessage: function(i, l, h) {
            ...
            var g = {
                "req.message.g.post": "im_g_send_message"
            };
            var m = g[f];
            var k = {};
            switch (l) {
            ...
            var j = this.post(new e(f, {
                message: n
            }), "default", "default").success(function(o) {
        ...
        postChatroomMessage: function(i, l, h) {
            ...
            var g = {
                "req.message.c.post": "im_c_send_message"
            };
            var m = g[f];
            var k = {};
            switch (l) {
        ...
        getCustomMessage: function(f) {
            return this.getMulti(new e("req.message.custom.get", {
                data: f
            }))
        },
        v: 1
...
M.closure(function(e) {
    ...
    function m() {
        ...
        c = setInterval(function() {
            if (v()) {
                f++;
                document.title = f % 2 === 0 ? "銆愩€€銆€銆€銆� - " + q : "銆愭柊娑堟伅銆� - " + q
            } else {
                document.title = q
            }
        }, h)
    ...
    a.prototype = {
        init: function() {
            ...
            this.verifyCon.delegate("._j_fre_confirm", "click", $.proxy(function(h) {
                ...
                var f = g.length;
                if (f == 0) {
                    this.showErro("鎮ㄨ繕娌℃湁杈撳叆楠岃瘉鐮�!");
                    return false
                } else {
                    if (f !== 6) {
                        this.showErro("璇疯緭鍏ユ纭殑楠岃瘉鐮�!");
                        return false
                    }
                }
                    ...
                    if (i) {
                        if (i.ret === 1) {
                            ...
                            if (i.ret === 0) {
                                ...
                                this.refreshImg();
                                this.showErro("杈撳叆鐨勯獙璇佺爜涓嶆纭�!")
                            } else {
                                if (i.ret === -1) {
                                    this.showErro("閿欒娆℃暟杩囧锛岃绋嶅€欏皾璇�")
                                }
                            }
                        }
                    }
...
M.define("FrequencySystemVerify", function(f, e, g) {
    ...
    var a = '<div class="popYzm" style="z-index:inherit;position: relative;width:350px;height: 250px"><div class="protectedYZM" style="border: none;box-shadow: none;padding:25px 15px;"><p>浜茬埍鐨勮渹铚傦紝浣犳搷浣滅殑閫熷害鏈夌偣鍍忔満鍣ㄤ汉<br>鏉ヨ瘉鏄庝笅鑷繁鍚</p><div class="YZMInput"><input class="_j_fre_text" type="text" placeHolder="楠岃瘉鐮�"></div><div class="YZMInput"><img src="http://images.mafengwo.net/images/home_new2015/verify.gif" width="150px" height="40px"><label><a href="javascript:void(0);" class="_j_change_img">鐪嬩笉娓咃紝鎹竴寮�</a></label></div><div class="YZMSubmit"><a href="javascript:void(0);" class="_j_fre_confirm" title="纭畾">纭畾</a><span class="n-error">閿欒娆℃暟杩囧锛岃绋嶅€欏皾璇�</span></div></div></div>';
...
M.closure(function(a) {
    M.log("鍙浣犳湁姊︽兂锛屽氨鍔犲叆鎴戜滑\n浣犲嵆灏嗚璇佷簰鑱旂綉鏈€鏂拌秼鍔跨殑蹇€熸垚闀縗n铓傝渹绐濈殑涓€鍒囪祫婧愰兘浼氭垚涓轰綘鎴愰暱璺笂鐨勬渶澶у姪鍔沑n浣犲彲浠ュ拰铓傝渹绐濅竴璧蜂功鍐欎簰鑱旂綉鐨勯浜戝杩筡n鍦ㄨ繖閲屾湁涓€缇ゅ拰浣犱竴鏍凤紝鐤媯鍦扮儹鐖变簰鑱旂綉鍜屾梾琛岀殑浜轰滑\n铓傝渹绐濊兘涓轰綘瀹炵幇姊︽兂鎻愪緵鏈€骞块様鐨勫钩鍙�");
    M.log("璇峰皢绠€鍘嗗彂閫佽嚦 %csuperhr@mafengwo.com%c锛� 閭欢鏍囬璇蜂互鈥淿console鈥濈粨灏撅級", "color:#4ae;", "color:inherit;");
    M.log("鑱屼綅浠嬬粛锛歨ttp://www.mafengwo.cn/s/hr.html")
});
M.closure(function(a) {
